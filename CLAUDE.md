...
| 21 | Play Billing (D67/D71) | Code complete (D106/D107); **Play product live**; awaiting a tester purchase | Server: `POST /v1/billing/play/verify` deployed to `dev` + `production`. Mobile: `expo-iap` purchase flow shipped; AAB versionCode 52 uploaded to the Play **internal** track automatically (`PlayServiceAccountKey` is set as a GitHub secret — `HAS_PLAY_KEY: true`). **Play Console done 2026-09-19:** subscription `techtok_plus` (“TechTok Plus”) with base plans `plus-monthly` (P1M) and `plus-yearly` (P1Y), both **Active**, 174 regions, priced €2.99 / €24.99 **net** — Play adds each country's VAT on top, so Austria displays €3.59 / €29.99. That is deliberate: it keeps D74's €2.54/mo net-after-Play's-15% arithmetic intact. Play App Signing was already enabled (key in use since 2026-09-02), so D68's SHA-1 exists now rather than at first upload. **Ordering trap found the hard way:** Play's Subscriptions page has no Create button until an *uploaded artifact* declares `com.android.vending.BILLING`, so the mobile half had to ship before the Console work — the reverse of the phase plan's task order. Still open: the subscription's benefits description (recommended, not required); grab the Play-managed SHA-1 into the Google Cloud OAuth Android client (D68); a license tester completing a real purchase end to end; the refund/cancel/restore acceptance checks; and the 14-day closed test with ≥12 testers. **Never run `expo prebuild` to add a native module here** — it reverts every committed `android/` customization and resets `versionCode` to 1 (D107). |
