...
| 21 | Play Billing (D67/D71) | Code complete, both halves (D106/D107); blocked on Play Console | Server: `POST /v1/billing/play/verify` behind an injectable Play API client in `core`, four-outcome `resolvePlayVerification`, purchase-token→user binding (migration `0004`), `PlayServiceAccountKey` as a **placeholder-defaulted** `sst.Secret` (503 when unset, so deploys never break). Mobile: `expo-iap` 5.6.2, real paywall CTAs with Play's own prices, launch-time `getAvailablePurchases()`→verify→acknowledge, restore, Manage-subscription deep link; free quota now **30/10** (D107 retires D75d). **Blocked, maintainer-only:** Play's Subscriptions page has no Create button until an **uploaded artifact declares `com.android.vending.BILLING`** — so this change must merge and ship an AAB (versionCode ≥ 52) *before* `techtok_plus` can be created. That inverts the phase plan, which listed Console setup as task 1. Then: create the subscription + base plans `plus-monthly`/`plus-yearly`, license testers, set `PlayServiceAccountKey` in both places, internal-track build, 14-day closed test. **Never run `expo prebuild` to add a native module here** — it reverts every committed `android/` customization and resets `versionCode` to 1 (D107). |
