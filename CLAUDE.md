...
| 14 | CI/CD hardening (D33–D35, D38, D40–D44) | Done, incl. one real E2E run | Maintainer: add the `AWS_E2E_ROLE_ARN` GitHub secret (D100 removed `AWS_DEV_DEPLOY_ROLE_ARN` along with the CI dev deploy). Mobile versioning is automated again: `mobile-build.yml` computes a conventional-commit bump baselined against the last `mobile-v*` tag and pushes it directly to `main` (D44, with `[skip ci]` + rebase-retry), then tags the release (D42) — no more hand-editing `apps/mobile/app.json` on mobile-relevant PRs unless overriding the auto-bump |
