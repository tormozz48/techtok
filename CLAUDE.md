...
| 21 | Play Billing (D67/D71) | Server half code complete (D106); mobile half next | Done: `POST /v1/billing/play/verify` (`purchases.subscriptionsv2.get` behind an injectable client in `core`, 11 recorded fixtures, no live calls), the four-outcome `resolvePlayVerification` decision layer, purchase-token→user binding (migration `0004`), `sha256(userId)` obfuscated account id, and `PlayServiceAccountKey` as a **placeholder-defaulted** `sst.Secret` so an unset value can't break `sst deploy` (route answers 503 instead). Product identity: one subscription `techtok_plus`, base plans `plus-monthly`/`plus-yearly`, ids pinned in `packages/shared/src/billing.ts` and **permanent once created in Play Console**. Still open: PR 2 (expo-iap purchase flow, launch-time `queryPurchases` → verify, restore, Manage-subscription deep link, and the free quota drop to 30/10); then all of task 1/2/5 — Play Console product + base plans, license testers, the `PlayServiceAccountKey` value in both places, and an internal-track build, which is the only way IAP can be exercised at all |
