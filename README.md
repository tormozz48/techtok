...
| CloudWatch + SNS + AWS Budget | Seven production-only alarms — DLQ depth (×3), Step Functions failure, API 5xx, ingest stalled, Lambda throttled — to one email-subscribed SNS topic; no ops dashboard (D88); $25/mo budget as an infra-drift signal (D74). |
