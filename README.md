...
| `NEON_DATABASE_URL_DEV_DIRECT` | Deploy dev | Direct (non-pooled) Neon connection string for `dev`; used only to run `pnpm db:migrate` (D92) — distinct from the pooled `NeonDatabaseUrl` `sst.Secret` the Lambdas link at runtime |
