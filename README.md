...
| `NEON_DATABASE_URL_DEV_DIRECT` | Deploy dev, E2E | Direct (non-pooled) Neon connection string for `dev`; runs `pnpm db:migrate` (D92) and, since D96, backs the E2E `backend-pipeline` job's source-freshness assertion — `dev` only, production's equivalent is never exposed to E2E — distinct from the pooled `NeonDatabaseUrl` `sst.Secret` the Lambdas link at runtime |
