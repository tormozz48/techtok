...
| D13 | Backend stack | SST v3 (Ion), Node 22, TypeScript strict | User choice; SST v3 is the current major | — |
...
**SQS `TransformQueue`** (visibility 90 s, redrive to DLQ after 3 receives) → **Transform Lambda** (batch ≤ 5, **reserved concurrency 2** — the LLM rate/cost valve):
