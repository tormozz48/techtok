...
- **Status:** agreed 2026-07-18, after Q&A session (decisions logged in §2); localization + compact-reader + UX extension agreed 2026-07-22 (D20–D25, phases 7–10); UI library, eager translation, image quality, digest retirement, and visual identity agreed 2026-07-24 (D26–D30, phases 11–12)
...
| **Bedrock: card translations** (D27, was D22) | **eager**, ~3 languages × every LLM-carded post (worst case ≈ 360/day if the 120/day transform cap is maxed), ~0.5k in + 0.4k out each (self-critique) | ~$27/mo if the transform cap is maxed daily — a real jump from D22's on-demand $7.5 at-cap figure, since volume now tracks *ingestion*, not reads; the `translations#<date>` cap must be raised to cover this and rechecked against the $10 alarm before phase 12 rollout (task 1) |
| **Bedrock: compact articles** (D23) | on-demand, 20/day cap, ~3.5k in + ~1k out each | ~$5 at cap; **typical ≤$1** (taps, not posts) |
| **Total** | | **theoretical all-caps-maxed ≈ $45–46/mo post-D27 (was ≈ $20–22 pre-D27) — this now exceeds the $10 alarm by a wide margin even before accounting for compact articles; typical was < $8 under D22 but D27's eager translation volume tracks ingestion, not reads, so "typical" and "at-cap" converge and the old typical-case comfort margin no longer applies** |

The AWS Budget alarm stays at **$10** (D11/D22) as the target, but D27's eager translation model breaks the "typical usage stays under it" comfort the D22 numbers relied on — **this is an explicitly unresolved tension, not a settled number**: Phase 12 task 1 is to raise `translations#<date>` to cover eager volume *and* recheck the whole model against this $10 target before rollout, likely by lowering the global `transforms#<date>` transform cap (fewer posts get LLM cards → proportionally fewer eager translations) rather than raising the budget itself (D11 reaffirmed). All four caps remain env vars, changeable without a code deploy.
Levers if over budget: lower the global transform cap (directly shrinks eager translation volume post-D27), tighten per-source quotas, shrink truncation, 60-min cadence, Bedrock batch API (−50%), or fall back to D27's documented demand-targeted alternative (eager translation only for languages with ≥1 active user).
Cost Explorer check one week after phases 8 and 9 ship is mandatory (see IMPLEMENTATION_PLAN.md phase 10); a second check is mandatory one week after phase 12 ships, specifically to validate the eager-translation cost model above against real numbers.
...
| Eager pre-translation cost overrun (D27) | All 3 non-English languages generated for every post regardless of demand; raised `translations#<date>` cap is rechecked against the $10/mo budget (D11) before rollout; fallback is demand-targeted eager translation (active languages only) if real cost data busts the ceiling |
