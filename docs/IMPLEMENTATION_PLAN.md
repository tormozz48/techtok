...
6. **Mobile:** `react-native-nitro-google-signin` (`GoogleOneTapSignIn`, Credential Manager under the hood — see D75 for why this library and not `@react-native-google-signin/google-signin`). No `expo-secure-store` needed — the SDK persists its own session (Credential Manager), so `authStore.restore()` calls `signIn()`'s low-friction attempt at app start instead of this app caching a token itself. `/auth` route gated via expo-router's `Stack.Protected`; `api/client.ts`'s `apiFetch` attaches `Authorization` and, on a 401, calls `refreshToken()` (a silent re-sign-in) and retries **once**; `/account` screen (email, sign out, delete account with a native confirm dialog). Zustand's device-UUID state is deleted.
