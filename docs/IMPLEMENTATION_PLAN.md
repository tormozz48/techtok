...
| 24 | Relational data layer (Neon + Drizzle) | Neon Postgres per stage (`NeonDatabaseUrl` secret), normalized 15-table schema, Drizzle over `neon-http`, PGlite test harness, DynamoDB removal | ~9 d + 2-week soak |
...
13. **Generate the upload keystore** (maintainer, credential): `keytool -genkeypair` per `docs/DISTRIBUTION.md`, stored outside the repo, with `apps/mobile/android/keystore.properties` filled in from the committed `.example`. `android/app/build.gradle` already reads it and falls back to the debug key when absent.
14. **Make the store artifact an AAB with a bound update channel.** `eas.json`'s `production` profile is currently `"buildType": "apk"` with `"distribution": "internal"` — wrong on both counts for Play. Change to `"buildType": "app-bundle"` and build via `eas build --platform android --profile production --local`. This matters beyond the file extension: the committed `AndroidManifest.xml` has `EXPO_UPDATES_CHECK_ON_LAUNCH=ALWAYS` and an update URL but **no channel meta-data** — `eas build` injects the channel from `eas.json`, so the `pnpm build:android` raw-Gradle path documented in `DISTRIBUTION.md` would ship an app polling for updates with no channel binding.
15. **Point CI's OTA publish at the store channel too.** `mobile-build.yml:211` publishes `eas update --channel preview`; a `production`-profile store build listens on `production` and would never see it (D65's mechanism, silently inert). Either publish to both channels or make the store release its own workflow — decide deliberately rather than discovering it when a hotfix does not arrive.
16. **Bake the production API URL:** `EXPO_PUBLIC_API_URL` must point at the `production` API, not `dev`. It is a `workflow_dispatch` input in CI and a local `.env` value otherwise — both are easy to get wrong silently. Add a pre-upload check that fails loudly on a non-production URL.
17. **Verify `targetSdkVersion`** (inherited from Expo SDK 57's root-project defaults via `rootProject.ext`) against Play's current requirement for new apps, and confirm `versionCode` has advanced past every prior upload — CI auto-bumps it on merge (D44), not on your publish schedule.
...
**Goal:** all four repos in `packages/core/src/repos/` are backed by a normalized Postgres schema on Neon, reached through Drizzle over the stateless HTTP driver, with **no change to any `packages/shared` contract and no change to any repo method signature** (D88). The DynamoDB tables stay deployed but unread through a soak window, then go.

**Why this is a data-shape phase, not a cost phase.** Measured against live AWS on 2026-08-31: `production` holds ~56 MB total (`Posts` 15,818 rows / 54.8 MB, `UserActivity` 2,117 / 1.1 MB, `Users` 8, `Sources` 11) and `dev` ~36 MB — comfortably inside DynamoDB's permanent free tier. Nothing here saves money. What it buys is a schema that enforces its own invariants, queries that can filter and join, and a real database in unit tests. The one genuinely new *cost* is Neon's 5 GB/month egress, which is the tightest constraint in the phase and is what makes the stage-5 filter pushdown mandatory rather than optional.
...
6. `packages/core/src/db/schema.ts` — the fifteen tables and seven enums of D88. Enums are built **from `@techtok/shared`**, not restated: `pgEnum('topic', TOPICS)`, `pgEnum('language', LANGUAGES)`, `pgEnum('transform_kind', transformKindSchema.options)`. That keeps the taxonomy single-source, the same principle as commit 8cfc339, and is the specific reason Drizzle beat Prisma here — a `.prisma` file would be a second place these lists live. Every declaration is a `const`, so the file is entirely group 1 under the file-organization rule.
...
28. **Push the feed filters into the query.** `buildFeed` today fetches 25 rows per topic across up to 12 topics and discards most client-side on `status`, `duplicateOf`, `mutedSourceIds` and `compactLangs`. Extend `BuildFeedDeps.queryByTopic`'s opts to carry those filters and let `posts_feed_idx` serve them; `buildFeed` keeps merging, ranking and read-exclusion and stops filtering. Its tests change accordingly.
29. **Select one language, not four.** With `post_translations` split out, the feed joins the requested language only and never touches `excerpt`, `s3RawKey`, `origTitle` or figures. Tasks 28 and 29 together take the wire cost from ~700 KB/request to ~48 KB — from about 7,500 feed requests per month against Neon's 5 GB allowance to about 110,000. **Neither is optional, and they must land in the same PR:** stage 5 shipping without them puts `production` over its egress cap inside a month at today's 8 users.
...
43. Soak two weeks. Watch, specifically: Neon storage as a percentage of 0.5 GB, CU-hours against 100/month, egress against 5 GB, and p95 query latency versus the DynamoDB baseline. **Hitting any free-plan limit suspends compute until the next billing cycle** — that is a hard outage, not a degrade, so alert well below each ceiling.
...
47. Update `infra/monitoring.ts`: the `AWS/DynamoDB` namespace alarm and the "DynamoDB consumed capacity" / "DynamoDB throttled requests" dashboard widgets have no equivalent once the tables are gone. Replace with what Neon actually exposes, or delete them deliberately — a dashboard widget silently reporting nothing is worse than no widget.
...
50. Update DESIGN §6 to describe the Postgres model, leaving the DynamoDB design in the D88 log entry as superseded history rather than deleting it.
...
- [ ] A real feed request on `dev` transfers ≲60 KB from Neon, measured — not assumed — confirming tasks 28–29 landed.
