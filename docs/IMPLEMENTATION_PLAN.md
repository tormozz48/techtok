...
| 24 | Relational data layer (Neon + Drizzle, D89) | Neon Postgres per stage (`NeonDatabaseUrl` secret), normalized 15-table schema, Drizzle over `neon-http`, PGlite test harness, DynamoDB removal | ~9 d + 2-week soak |
...
6. `packages/core/src/db/schema.ts` — the fifteen tables and seven enums of D89. Enums are built **from `@techtok/shared`**, not restated: `pgEnum('topic', TOPICS)`, `pgEnum('language', LANGUAGES)`, `pgEnum('transform_kind', transformKindSchema.options)`. That keeps the taxonomy single-source, the same principle as commit 8cfc339, and is the specific reason Drizzle beat Prisma here — a `.prisma` file would be a second place these lists live. Every declaration is a `const`, so the file is entirely group 1 under the file-organization rule.
...
50. Update DESIGN §6 to describe the Postgres model, leaving the DynamoDB design in the D89 log entry as superseded history rather than deleting it.
