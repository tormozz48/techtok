...
14. ✅ **Make the store artifact an AAB.** `eas.json`'s `production` profile was `"buildType": "apk"` / `"distribution": "internal"` — wrong on both counts for Play (an AAB isn't directly installable the way `internal` distribution assumes; `store` is the matching EAS concept for a Play-bound artifact). Now `"buildType": "app-bundle"` / `"distribution": "store"`; build via `eas build --platform android --profile production --local`.
