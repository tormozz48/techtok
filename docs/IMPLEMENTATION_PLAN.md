...
| 24 | Relational data layer (Neon + Drizzle, D90) | Neon Postgres per stage (`NeonDatabaseUrl` secret), normalized 16-table schema, Drizzle over `neon-http`, PGlite test harness, DynamoDB removal | ~9 d + 2-week soak |
...
3. ✅ **Legal/compliance surface — built.** `apps/site/src/pages/privacy.astro` and `delete-account.astro` are live in the repo and linked from the site footer, and [docs/DATA_SAFETY.md](DATA_SAFETY.md) holds the Play Data Safety answer key traced field-by-field to the code. The project domain `techtokapp.eu` and its real `privacy@`/`support@` mailbox are provisioned (D103), so `CONTACT_EMAIL` is not a placeholder. **Still open:** the GitHub Pages custom-domain click (Settings → Pages, then Enforce HTTPS once the certificate issues), and linking the privacy policy from the *app's* settings screen — the site footer links exist, the in-app link does not. Terms of service are not required for a free app and land with item 5.
...
2. **Quota counters (D69):** `quota { day, cardReads, readerOpens }`, incremented atomically with a day-rollover reset against the user's stored IANA timezone. *(As shipped and then ported by D94, this is a `user_quotas` table keyed `unique(user_id, day)` — a new day is simply a new row, which deleted the conditional-rollover dance this task describes. See DESIGN §6.)* **Do not resurrect the `Counters` table** (D31). Watch for DynamoDB reserved keywords in the `UpdateExpression` — `day` and `status` are both reserved, and `aws-sdk-client-mock` will not catch it (the bug that bit phases 2 and 8).
...
5. **Ops script:** `scripts/grantEntitlement.ts` to grant/revoke `plus` by `userId` — how the maintainer tests the paid path in this phase and comps accounts forever after. Built as a local CLI tool (not a deployed Lambda) — calls `UsersRepo.grantEntitlement` directly, the exact same write path Play's verify callback will use in phase 21. *(Built as described, with one change post-D90: both this and phase 19's `scripts/wipeUsers.ts` now take a `--stage` plus a Neon `DATABASE_URL` and go through `createSqlClient`. The shared `scripts/lib/discoverTableName.ts` helper this task called for was never needed — it existed to find DynamoDB physical table names by tag, and there are no tables to discover any more.)*.
...
3. **Run the D68 wipe:** `pnpm wipe-users -- --stage dev`, then `--confirm`; same for `production`. Follow CLAUDE.md's Destructive Operations rule — export to a scratchpad backup and print exact row counts before confirming. *(Post-D90 the script takes a Neon `DATABASE_URL` and clears the user-scoped Postgres tables; the `on delete cascade` FKs mean deleting a `users` row takes its six child tables with it. Note phase 24's migration `0003` already dropped and recreated every table on both stages, so there may be nothing left to wipe — check the counts before assuming this step is still needed.)*
...
19. ✅ **Play developer account registered** ($25, personal per D75) and identity verification (KYC) passed.
...
> **Status: done** (D90/D93/D94, landed through 2026-09-02; the DynamoDB decommission landed 2026-09-04). Two things happened differently from the plan below and are recorded here rather than silently rewritten:
>
> - **Stage 7 was not executed as written.** Rather than reshaping 15,818 posts and 2,117 activity rows into the new schema, **all existing data was dropped** — the maintainer's explicit call. Migration `0003_integer_keys.sql` drops and recreates every table and runs itself on both stages via D92. `scripts/migrateToNeon.ts` was therefore never written, and stage 35's pre-flight constraint audit never ran because there was nothing to preserve. The cost argument in task 40 (re-deriving those posts means re-paying for 1 card + 3 translations + 4 compacts each) was accepted and overridden deliberately, not overlooked.
> - **`aws-sdk-client-mock` is still a `packages/core` dev dependency**, contrary to task 45 and the matching acceptance criterion. It no longer mocks any repo — the four Postgres repos test against PGlite — but the S3 and SQS stores (`contentStore`, `imageStore`, `rawArticleStore`, and the three queue repos) still use it, and those are out of this phase's scope. The criterion was written assuming the mock existed only for DynamoDB.
