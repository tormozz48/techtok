...
import { DEVICE_ID_HEADER, meResponseSchema, topicsPrefsRequestSchema } from '@techtok/shared';
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  const parsedBody = topicsPrefsRequestSchema.safeParse(rawBody);
  ...
  const user = await getRepo().updateTopics(deviceId, parsedBody.data.topics);
